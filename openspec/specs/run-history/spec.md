...
Managers can browse historical reviewer runs (list + detail), see per-project timing/errors, and for finished MR runs inspect skip reasons derived from `eligible_mrs.json`. The dashboard exposes up to five recent runs as an entry point.
